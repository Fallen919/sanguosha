...
Item {
    ...
    Item {
        ...
        PlayerArea {
            ...
            onKurouRequested: {
                ...
                if (wujiang) {
                    // 调用苦肉技能
                    wujiang.jineng1(currentPlayer, gameManager);
                }
            }
    ...
    Component.onCompleted: {
        if (gameManager) {
            ...
            // 设置玩家1的武将名称
            var player1 = gameManager.getdangqianplayer();
            if (player1 && player1.getwujiang()) {
                player1Area.wujiangName = player1.getwujiangming();
                console.log("玩家1武将:", player1Area.wujiangName);
            }

            topDiscardCard = gameManager.getTopDiscardCard();

            // 添加手牌重置信号连接
            gameManager.handCardsReset.connect(function() {
                console.log("手牌重置信号接收");
                handCardsModel.clear();
                var handCards = gameManager.getHandCards();
                for (var i = 0; i < handCards.length; i++) {
                    var cardData = JSON.parse(JSON.stringify(handCards[i]));
    ...
    function resetTargetSelection() {
        ...
        selectedCardIndex = -1; // 新增：重置选中的卡牌索引
    }
